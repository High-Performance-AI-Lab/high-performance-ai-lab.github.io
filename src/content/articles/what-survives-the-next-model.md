...
That last sentence rests on assumptions the model makes explicit: the reading is exact, the intent was authorized, the retry itself succeeds, no other request can commit the same intent after the reading, and the outcome is judged within a stated horizon. The policy that satisfies every world is “look, then act.” Nothing learned it. We wrote it, and the finite check confirms it under those assumptions.
...
*For this authorized intent, an exact authoritative reading shows zero effects. If no earlier or concurrent request for that intent can commit after the read, and one retry succeeds once, exactly one refund occurs within the stated horizon.*

Every clause is a premise, and the one-effect-per-successful-retry clause is a rule of our authored model, not a promise any real provider has made. Change the provider and the clause “no earlier or concurrent request can commit after the read” is open. The conclusion has not been falsified. Its next use needs evidence about that open clause. The investigator's task is to find a sufficient check at the lowest expected total cost, and to say when the available experiments cannot settle it.
...
The hard part is the missing entries. An expert can write “the provider never commits late” into a dependency list after the fifth world has been found. The research question is whether an investigator can find that clause before the duplicate refund, from the trajectory it observes, without being told which mechanism to look for. Our finite example exposes the missing mechanism. It does not detect it. Detection is the first capability such an investigator would need, and the first place the hypothesis can fail.
...
Repeating a measurement that makes identical predictions under the competing explanations cannot distinguish those explanations. Only a new observation or intervention on which their predictions differ can. Regrade the same saved run with a second evaluator, and the explanations that blame the checker predict a changed verdict. Rerun the agent with the acknowledgment delayed but its final value preserved, and the explanations that blame early stopping predict a changed outcome. Each explanation commits to a prediction before the intervention runs. An explanation fitted to a result is a hypothesis. A prediction on new evidence puts it at risk.

This is old science. Experimental design has selected informative tests for a century, and [active evaluation](https://arxiv.org/abs/2410.05952) already applies it to choosing which benchmark items to run. We want to make an earlier conclusion's premises the target: choose the experiment for what it can establish about them, and charge its cost against the value of keeping that conclusion. Memory then participates in experimental design. A [science-sandbox study](https://arxiv.org/abs/2608.30165) in hidden-rule worlds found that agents can improve a numerical objective without recovering the rule that produced it. Whether a recovered rule transfers better under change than a score-optimized policy is not established by that study. A useful test of our view would have to measure that transfer.

There is a second decision the investigator has to make: when to stop. Every experiment costs something, and combinations of experiments can be informative where each alone is not. The eval we want would choose when to buy more evidence and state what remains unexamined when it stops. Judge-panel research has begun to [formalize that stopping choice](https://arxiv.org/html/2608.19802v1) for a fixed set of instruments. Extending it to experiments the investigator generates is open.
...
A fair comparison needs a fixed cohort of scoped conclusions and a registered stream of changes: model swaps, tool upgrades, workload shifts, evaluator repairs, and worlds with omitted mechanisms. Give each method the same model access, known dependencies and changes. Audit what it keeps and what it refuses against a separately authored reference. Compare the learned investigator with full re-evaluation, reuse on unchanged identities, competent explicit dependency checks, and ordinary retrieval over past runs. Charge each for generation, qualification, failed investigations, human review and reruns.

The output should be a claim-reuse profile: which conclusions remained supported without new evidence, which needed a new check, which were withdrawn, and which were wrongly reused or wrongly refused. Keep retention and re-establishment separate, and count errors in both. Measure harmful reuse among reused claims and wrongful refusal among valid claims. An investigator that reuses nothing has an unestimated reuse error rate, not zero risk.
...
This gives the eval work to do before the next model exists: make explicit what it expects to survive, and why. That is a design position, and explicit dependency checks could implement it today. Our further hypothesis, the one that can fail, is that a learned investigator will discover the missing conditions and choose the next check better than those explicit checks, at matched error limits and lower total cost.
...
Part I argued that our understanding of the work should compound across models. Here is what we want that to mean in practice: discovering a condition nobody listed, recognizing when a conclusion's support changes, and choosing evidence that makes its next use justified. Our bet is that the loop can learn to do this. An eval earns that claim when what it leaves behind helps its successor investigate better.

The Red Queen was right that the ground moves. She was wrong that running is the point. The point is to know, after all the running, which of the things we established are still where we left them.
